...
typedef enum SceModuleAttribute {
 ...
 * module can have.
 */
typedef enum SceModulePrivilegeLevel {
    /** Lowest permission. */
 ...
 *
 * @returns A pointer to the `SceModule` structure if found, otherwise `NULL`.
 *
 * @attention Requires linking to `psploadcore_kernel` stub to be available.
 */
 ...
 *
 * @param addr An address somewhere within the module.
 *
 * @returns A pointer to the `SceModule` structure if found, otherwise `NULL`.
 *
 * @attention Requires linking to `psploadcore_kernel` stub to be available.
 ...
 *
 * @param buf A pointer to the file's contents.
 * @param exec_info A pointer to the executionInfo belonging to that executable.
 *
 * @return `0` on success.
 *
 * @attention Requires linking to `psploadcore_kernel` stub to be available.
 */
int sceKernelCheckExecFile(void *buf, SceLoadCoreExecFileInfo *exec_info);
 ...
 * @attention Requires linking to `psploadcore_kernel` stub to be available.
 */
int sceKernelProbeExecutableObject(void *buf, SceLoadCoreExecFileInfo *exec_info);

/**
 * Receive a list of UIDs of loaded modules.
 *
 * @param mod_id_list A pointer to a SceUID array which will receive the UIDs of the loaded modules.
 * @param size The size of `mod_id_list`. Specifies the number of entries that can be stored into mod_id_list.
